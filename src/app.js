...
function applyDailyStatusMeta(status) {
  ...
  if (elements.runMetaDailyStatus) {
    elements.runMetaDailyStatus.textContent = dailyStatusLabel(status);
    elements.runMetaDailyStatus.className =
  ...
  if (elements.runMetaDailyAt) {
    ...
    elements.runMetaDailyAt.textContent =
      finished !== "--" || generated !== "--"
        ? `${finished !== "--" ? finished : "--"}${generated !== "--" ? ` · 抓取 ${generated}` : ""}`
        : "--";
  }
}
...
async function bootstrapHistoryView() {
  ...
  const [seed, seedAi, dailyStatus] = await Promise.all([loadSeedHistory(), loadSeedAi(), loadDailyStatus()]);
  aiSeedByDate = seedAi || {};
